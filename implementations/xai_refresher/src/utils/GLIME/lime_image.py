...
from .utils.generic_utils import generate_samples
...
class LimeImageExplainerGLIME(object):
        ...
        for iter in range(iterations):
            ...
            if distribution == "uniform_adaptive_weight":
                ...
                def kernel(d, kernel_width):
                    return np.sqrt(
                        np.exp(
                            (len(np.unique(segments)) / 2 - (d**2)) / kernel_width**2
                        )
                    )
            ...
            top_labels_list_list = ret_exp.local_exp[ret_exp.top_labels[0]][
    ...
    def data_labels(
        ...
        labels = []
        if (
            ("gaussian_additive" in distribution)
            or ("smooth_grad" in distribution)
            or ("local_uniform" in distribution)
            or ("laplace" in distribution)
        ):
            data[0, :] = 0
        else:
            data[0, :] = 1
        ...
        for row in rows:
            ...
            if distribution in ["gaussian_isotropic"]:
                for ii in range(len(segment_id)):
                    temp[segments == segment_id[ii]] = (
                        temp[segments == segment_id[ii]] * row[ii]
                    )
