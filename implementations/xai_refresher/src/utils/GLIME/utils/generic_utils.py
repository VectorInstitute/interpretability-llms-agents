...
def compute_hisogram(img, segment):
    ...
    hists = dict.fromkeys(range(max_id))
...
def edge_affinity(x, segment):
    ...
    for ii in range(W.shape[0]):
        ...
        for jj in range(W.shape[1]):
            if jj not in W_[ii]:
                continue
            W[ii, jj] = W_[ii][jj] * np.exp(
                -(chi_square_distances[ii, jj] ** 2) / sigma_H
            )
    return W
...
def generate_samples(
    ...
    if distribution == "uniform":
        samples_arr = random_state.randint(0, 2, num_samples * num_features).reshape(
        ...
        for ii in range(len(segment_id)):
            ...
            max_scale.append(255.0 / max_x)

        for ii in range(num_samples):
            sample = random_state.normal(
                loc=np.ones(len(segment_id)),
                scale=kernel_width * np.ones(len(segment_id)),
            )
        ...
        for ii in range(num_samples):
            sample = (
                ...
                * kernel_width
                * random_state.normal(
                    loc=np.zeros(len(segment_id)), scale=np.ones(len(segment_id))
                )
            )
            for idx in range(len(segment_id)):
        ...
        if weight_method == "outsim":
            device = next(model.parameters()).device
            original_out = (
                model(get_preprocess_transform()(x).unsqueeze(0).to(device))
                .detach()
                .cpu()
                .numpy()
            )
        ...
        for ii in range(num_features):
            ...
            if weight_method == "outsim":
                masked_out = (
                    model(
                        get_preprocess_transform()(masked_image).unsqueeze(0).to(device)
                    )
                    .detach()
                    .cpu()
                    .numpy()
        ...
        probs = np.array(
            [
                comb(num_features, i)
                * (2 ** (-num_features))
                * np.exp((i - num_features) / (kernel_width**2))
                for i in range(num_features)
            ]
        )
        probs /= probs.sum()
        sample_length = random_state.choice(
            range(num_features), size=num_samples, p=probs
        )
        ...
        if distribution == "exp":
            ...
            probs = np.array(
                [
                    comb(num_features, i) * (2 ** (-num_features))
                    for i in range(num_features)
                ]
            )
        ...
        for i in range(num_samples):
            idx = random_state.choice(
                range(num_features), size=sample_length[i], replace=False
            )
            zeros = np.zeros(num_features)
            zeros[idx] = 1
            samples_arr.append(zeros)
...
def fit(X, y, kernel_width=1, fit_intercept=True, sample_weight=None, copy_X=True):
    X, y, X_offset, y_offset = preprocess_data(
        X, y, fit_intercept, copy=copy_X, sample_weight=sample_weight
    )
    ...
    A_d = np.sum(
        [
            comb(num_features, kk) * np.exp(kk / (kernel_width**2))
            for kk in range(num_features + 1)
        ]
    )
    alpha_1 = (
        np.sum(
            [
                comb(num_features - 1, kk) * np.exp((kk + 1) / (kernel_width**2))
    ...
    alpha_2 = (
        np.sum(
            [
                comb(num_features - 2, kk) * np.exp((kk + 2) / (kernel_width**2))
                for kk in range(num_features - 1)
            ]
        )
        / A_d
    ...
    Sigma_inv = 1.0 / sigma_0 * (sigma_2 * ones + (sigma_1 - sigma_2) * eyes)
    x_y = (X * y).sum(axis=0)
    Gamma = (
        np.linalg.inv(
            Sigma_inv.T @ X.T @ X @ Sigma_inv + 1.0 / X.shape[0] * np.eye(num_features)
        )
        @ Sigma_inv.T
        @ x_y
    )
    ...
    return (
        intercept,
        sorted(
            zip(range(num_features), coef), key=lambda x: np.abs(x[1]), reverse=True
        ),
        score,
        np.sum(coef) + intercept,
    )
