...
class eac:
    ...
    def explain_instance(
        ...
        data, data_raw = load_and_preprocess_image(
            image_path, config, plot=False, model=model_explain_processor
        )
        data_transformed = np.transpose(data, (1, 2, 0))
        self.data = data

        pred_image_class = predict_image_(
            image_path, model, config, True, model_processor=model_explain_processor
        )[1]
...
def evaluate_explanation(
    ...
    def shuffle_weights_tensor(weight_tensor):
        ...
        shuffled_weights = flattened_weights[permuted_indices].view(
            weight_tensor.size()
        )
    ...
    def lime_segmentation(image, config):
        ...
        if config["lime_segmentation"]["slic"]:
            segments = slic(
                image,
                n_segments=config["lime_segmentation"]["num_segments"],
                compactness=config["lime_segmentation"]["slic_compactness"],
            )
        elif config["lime_segmentation"]["quickshift"]:
            segments = quickshift(
                image,
                kernel_size=config["lime_segmentation"]["kernel_size"],
                max_dist=config["lime_segmentation"]["max_dist"],
                ratio=0.1,
            )
            ...
            segments = felzenszwalb(
                ...
                scale=100,
                sigma=0.2,
                min_size=config["lime_segmentation"]["min_size"],
            )
        elif config["lime_segmentation"]["watershed"]:
            segments = watershed(
                image, markers=config["lime_segmentation"]["markers"], compactness=0.01
            )
            segments_new = np.zeros([segments.shape[0], segments.shape[1]]).astype(
                np.uint8
            )
    ...
    def segmentation_detr(image_raw, feature_extractor, model, dim=(380, 380)):
        ...
        resized_panoptic_seg = cv2.resize(
            panoptic_seg, dim, interpolation=cv2.INTER_LINEAR
        )
    ...
    def top_outliers(arr):
        ...
        # Identify outliers
        outliers = [
            (i, x) for i, x in enumerate(arr) if abs(x - mean) > threshold * std
        ]
    ...
    def segment_sam(image_path, dim, feature_extractor):
        ...
        for num, mask in enumerate(small_mask):
            int_array = np.zeros(
                (mask["segmentation"].shape[0], mask["segmentation"].shape[1])
            )
        ...
        resized_panoptic_seg = create_mask_sam(
            resized_panoptic_seg, hierarchical_dict, iteration=0
        )
        resized_panoptic_seg_nums = np.unique(resized_panoptic_seg)

        for old, new in zip(
            resized_panoptic_seg_nums, np.arange(len(resized_panoptic_seg_nums))
        ):
    ...
    def replace_background(
        image_path, background_path, id, config, dim=(380, 380), data_driven=False
    ):
        """
        ...
        if data_driven:
            resized_panoptic_seg = segment_seed_dynamic(
                data,
                image_path,
                config,
        ...
        fraction_altered = altered_count / (
            resized_panoptic_seg.shape[0] * resized_panoptic_seg.shape[1]
        )
    ...
    def predict_merge_data(
        ...
        if config["model_to_explain"]["EfficientNet"] or contrastivity:
            ...
            masked_image_ = model_explain_processor(
                images=masked_image, return_tensors="pt"
            )
            # Apply the mask
            if torch.cuda.is_available():
    ...
    def plot_image_background_to_save(data_local, mask):
        if config["model_to_explain"]["EfficientNet"]:
            ...
            config["model_to_explain"]["ResNet"]
            or config["model_to_explain"]["VisionTransformer"]
            or config["model_to_explain"]["ConvNext"]
        ):
    ...
    if config["model_to_explain"]["EfficientNet"]:
        ...
        data_transformed = data_raw.resize(
            (data_transformed.shape[0], data_transformed.shape[1])
        )
    else:
    ...
    groundtruth["Groundtruth"] = predict_image(
        image_path, model, config, False, model_processor=model_explain_processor
    )[0][0]
    if config["evaluation"]["target_discrimination"]:
        contrastivity["Groundtruth_test"] = predict_image(
            image_path,
            model_test,
            config,
            False,
            dim=(300, 300),
            model_processor=model_explain_processor,
            contrastivity=True,
        )[0][0]
    ...
    if config["model_to_explain"]["EfficientNet"]:
        ...
        for layer in model_shuffle.layers:  # Check
            ...
            if isinstance(layer, tf.keras.layers.Dense):  # Shuffle the weights
                shuffled_weights = shuffle_weights(
                    layer.get_weights()
                )  # Set the shuffled weights back to the layer
    ...
    if config["XAI_algorithm"]["DSEG"]:
        data_, data_raw = load_and_preprocess_image(
            image_path, config, plot=False, model=model_explain_processor
        )
        ...
        with HiddenPrints():
            ...
            explanation_dseg = explainer_dseg.explain_instance(
                data,
                model_p,
                segmentation_algorithm[0],
                segmentation_algorithm[1],
            ...
            if config["lime_segmentation"]["adaptive_num_features"]:
                while local_features > 1:
                    ...
                    temp_normal_dseg, mask_normal_dseg = (
                        explanation_dseg.get_image_and_mask(
                            explanation_dseg.top_labels[0],
                    ...
                    dseg_prediction, data_ = predict_merge_data(
                        image_path,
                        model,
                        mask_normal_dseg,
                ...
                labels, values, _ = zip(
                    *explanation_dseg.local_exp[explanation_dseg.top_labels[0]]
                )
                ...
                num_features_dseg = config["lime_segmentation"][
                    "num_features_explanation"
                ]

            temp_normal_dseg, mask_normal_dseg = explanation_dseg.get_image_and_mask(
                explanation_dseg.top_labels[0],
                positive_only=config["lime_segmentation"]["positive_only"],
            ...
            if config["model_to_explain"]["EfficientNet"]:
                ...
                dim = (
                    data["pixel_values"][0].shape[1],
                    data["pixel_values"][0].shape[2],
                )
            ...
            plt.savefig(
                to_save_path
                + image_path.rsplit(".", 1)[0].split("/")[-1]
                + "/dseg_segmentation.png",
                bbox_inches="tight",
                dpi=1200,
                pad_inches=0,
            )

            labels_dseg, values_dseg, _ = zip(
                *explanation_dseg.local_exp[explanation_dseg.top_labels[0]]
            )
            ...
            if not config["lime_segmentation"]["fit_segmentation"]:
                ...
                config["lime_segmentation"]["num_segments"] = len(
                    np.unique(values_dseg)
                )
            ...
            while len(
                np.unique(lime_segmentation(data_segmentation, config))
            ) not in range(dseg_segments - to_sub, dseg_segments + 5):
                ...
                if current_segments < dseg_segments - to_sub:
                    ...
                    if config["lime_segmentation"]["slic"]:
                        config["lime_segmentation"]["num_segments"] = (
                            2 * config["lime_segmentation"]["num_segments"]
                        )
                    elif config["lime_segmentation"]["quickshift"]:
                        config["lime_segmentation"]["kernel_size"] = (
                            config["lime_segmentation"]["kernel_size"] - 2
                        )
                    elif config["lime_segmentation"]["felzenszwalb"]:
                        config["lime_segmentation"]["min_size"] = int(
                            0.5 * (config["lime_segmentation"]["min_size"])
                        )
                    elif config["lime_segmentation"]["watershed"]:
                        config["lime_segmentation"]["markers"] = (
                            2 * config["lime_segmentation"]["markers"]
                        )
                    pos_loop = True
                elif current_segments >= dseg_segments + 5:
                    print("greater", current_segments, dseg_segments)
                    if config["lime_segmentation"]["slic"]:
                        config["lime_segmentation"]["num_segments"] = int(
                            0.5 * (config["lime_segmentation"]["num_segments"])
                        )
                    elif config["lime_segmentation"]["quickshift"]:
                        config["lime_segmentation"]["kernel_size"] = (
                            config["lime_segmentation"]["kernel_size"] + 2
                        ...
                        config["lime_segmentation"]["min_size"] = (
                            2 * config["lime_segmentation"]["min_size"]
                        )
                    elif config["lime_segmentation"]["watershed"]:
                        config["lime_segmentation"]["markers"] = int(
                            0.5 * (config["lime_segmentation"]["markers"])
                        )
            ...
            to_save_dseg = plot_image_background_to_save(
                data_transformed, mask_normal_dseg
            )
        ...
        print(
            "LIME Segments: ",
            len(np.unique(lime_segmentation(data_segmentation, config))),
        )
    ...
    if config["XAI_algorithm"]["LIME"]:
        ...
        with HiddenPrints():
            ...
            explanation_lime_fix = explainer_Lime.explain_instance(
                data,
                model_p,
                None,
                None,
            ...
            if config["lime_segmentation"]["adaptive_num_features"]:
                ...
                while local_features > 1:
                    local_features = local_features - 1
                    temp_normal_lime, mask_normal_lime = (
                        explanation_lime_fix.get_image_and_mask(
                            explanation_lime_fix.top_labels[0],
                            positive_only=config["lime_segmentation"]["positive_only"],
                            num_features=local_features,
                            hide_rest=config["lime_segmentation"]["hide_rest"],
                        )
                    ...
                    dseg_prediction, data_ = predict_merge_data(
                        image_path, model, mask_normal_dseg
                    )

                    if dseg_prediction[0][0] != groundtruth["Groundtruth"][0]:
                        config["lime_segmentation"]["num_features_explanation"] = (
                            local_features + 1
                        )
                        break
                    config["lime_segmentation"]["num_features_explanation"] = (
                ...
                labels, values, _ = zip(
                    *explanation_lime_fix.local_exp[explanation_lime_fix.top_labels[0]]
                )
            ...
            temp_normal_lime, mask_normal_lime = (
                explanation_lime_fix.get_image_and_mask(
                    explanation_lime_fix.top_labels[0],
                    positive_only=config["lime_segmentation"]["positive_only"],
                    num_features=config["lime_segmentation"][
                        "num_features_explanation"
                    ],
                    hide_rest=config["lime_segmentation"]["hide_rest"],
                )
            ...
            if config["model_to_explain"]["EfficientNet"]:
                ...
                data_transformed_lime = data.copy()
                data_transformed_lime = np.array(
                    data_transformed_lime["pixel_values"][0]
                )
                data_transformed_lime = data_transformed_lime.transpose(1, 2, 0)
            ...
            if config["evaluation"]["preservation_check"]:
                lime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_normal_lime,
                    config=config,
            ...
            to_save_lime = plot_image_background_to_save(
                data_transformed, mask_normal_lime
            )
    ...
    if config["XAI_algorithm"]["BayesLime"]:
        ...
        with HiddenPrints():
            ...
            if not config["lime_segmentation"]["all_dseg"]:
                explanation_BayesLime = explainer_BayesLime.explain_instance(
                    data,
                    model_p,
                    None,
                    None,
            ...
            temp_normal_BayesLime, mask_normal_BayesLime = (
                explanation_BayesLime.get_image_and_mask(
                    explanation_BayesLime.top_labels[0],
                    positive_only=config["lime_segmentation"]["positive_only"],
                    num_features=config["lime_segmentation"][
                        "num_features_explanation"
                    ],
            ...
            labels_BayesLime, values_BayesLime, _ = zip(
                *explanation_BayesLime.local_exp[explanation_BayesLime.top_labels[0]]
            )
            ...
            if config["evaluation"]["preservation_check"]:
                BayesLime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_normal_BayesLime,
                    config=config,
                    model_explain_processor=model_explain_processor,
                )
                plt.imshow(data_)
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_check_BayesLime.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
                output_completness["BayesLIME_preservation_check"] = (
                    BayesLime_prediction
                )
            ...
            if config["evaluation"]["target_discrimination"]:
                ...
                contrastivity["BayesLIME_preservation_check_contrast"] = (
                    BayesLime_prediction
                )

            if config["evaluation"]["deletion_check"]:
                mask_normal_BayesLime_inverse = 1 - np.array(
                    mask_normal_BayesLime
                ).astype(bool)
                BayesLime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_normal_BayesLime_inverse,
                    config=config,
                    model_explain_processor=model_explain_processor,
                )
                plt.imshow(data_)
            ...
            if config["evaluation"]["target_discrimination"]:
                ...
                contrastivity["BayesLIME_deletion_check_contrast"] = (
                    BayesLime_prediction
                )

            to_save_lime = plot_image_background_to_save(
                data_transformed, mask_normal_BayesLime
            )
            plt.imshow(to_save_lime)
            plt.axis("off")
            plt.savefig(
    ...
    if config["XAI_algorithm"]["SLIME"]:
        ...
        with HiddenPrints():
            start_lime = time.time()
            if not config["lime_segmentation"]["all_dseg"]:
                explanation_SLIME = explainer_SLIME.explain_instance(
                    data,
                    model_p,
                    None,
                    None,
            ...
            temp_normal_SLIME, mask_normal_SLIME = explanation_SLIME.get_image_and_mask(
                explanation_SLIME.top_labels[0],
                positive_only=config["lime_segmentation"]["positive_only"],
                num_features=config["lime_segmentation"]["num_features_explanation"],
                hide_rest=config["lime_segmentation"]["hide_rest"],
            )

            labels_SLIME, values_SLIME, _ = zip(
                *explanation_SLIME.local_exp[explanation_SLIME.top_labels[0]]
            )
            ...
            if config["evaluation"]["preservation_check"]:
                slime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_normal_SLIME,
                    config=config,
            ...
            to_save_lime = plot_image_background_to_save(
                data_transformed, mask_normal_SLIME
            )
            plt.imshow(to_save_lime)
            plt.axis("off")
            plt.savefig(
    ...
    if config["XAI_algorithm"]["GLIME"]:
        ...
        with HiddenPrints():
            start_lime = time.time()
            if not config["lime_segmentation"]["all_dseg"]:
                explanation_GLIME = explainer_GLIME.explain_instance(
                    data,
                    model_p,
                    None,
                    None,
            ...
            temp_normal_GLIME, mask_normal_GLIME = explanation_GLIME.get_image_and_mask(
                explanation_GLIME.top_labels[0],
                positive_only=config["lime_segmentation"]["positive_only"],
                num_features=config["lime_segmentation"]["num_features_explanation"],
                hide_rest=config["lime_segmentation"]["hide_rest"],
            )

            labels_GLIME, values_GLIME = zip(
                *explanation_GLIME.local_exp[explanation_GLIME.top_labels[0]]
            )
            ...
            if config["evaluation"]["preservation_check"]:
                glime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_normal_GLIME,
                    config=config,
            ...
            to_save_lime = plot_image_background_to_save(
                data_transformed, mask_normal_GLIME
            )
            plt.imshow(to_save_lime)
            plt.axis("off")
            plt.savefig(
    ...
    if config["XAI_algorithm"]["EAC"]:
        ...
        with HiddenPrints():
            ...
            eac_explainer.explain_instance(
                image_path,
                model_p,
                segmentation_algorithm[1],
                model_explain_processor,
            ...
            if config["lime_segmentation"]["adaptive_num_features"]:
                while local_features > 1:
                    ...
                    eac_prediction, data_ = predict_merge_data(
                        image_path,
                        model,
                        mask_normal,
                        model_explain_processor=model_explain_processor,
        ...
        if config["XAI_algorithm"]["DSEG"]:
            with HiddenPrints():
                ...
                for iteration in range(config["evaluation"]["repetitions"]):
                    ...
                    explanation_dseg_ = explainer_dseg.explain_instance(
                        data,
                        model_p,
                        segmentation_algorithm[0],
                        segmentation_algorithm[1],
                    ...
                    labels_dseg, values_dseg, _ = zip(
                        *explanation_dseg_.local_exp[explanation_dseg_.top_labels[0]]
                    )

                    for j in range(len(labels_dseg)):
                        if labels_dseg[j] not in results_dictionary:
                            # Directly assign the value if the label is new
                            results_dictionary[labels_dseg[j]] = np.array(
                                [values_dseg[j]]
                            )
                        else:
                            # Concatenate the new value in a proper way
                            results_dictionary[labels_dseg[j]] = np.concatenate(
                                (results_dictionary[labels_dseg[j]], [values_dseg[j]])
                            )
                    ...
                    for category, values in results_dictionary.items():
                        ...
                        for observation in values:
                            df_data.append(
                                {"Superpixel": category, "Importance": observation}
                            )
                    ...
                    filtered_dict = {
                        k: v for k, v in df_dict.items() if len(v) == most_common_length
                    }
                    ...
                    to_save = sns.catplot(
                        x="Superpixel", y="Importance", data=df, kind="box", aspect=3
                    )
                    ...
                    plt.xticks(
                        selected_positions, selected_labels, rotation=45, fontsize=10
                    )  # Set the fontsize here as well
        ...
        if config["XAI_algorithm"]["LIME"]:
            with HiddenPrints():
                ...
                for iteration in range(config["evaluation"]["repetitions"]):
                    ...
                    labels_lime, values_lime, _ = zip(
                        *explanation_lime.local_exp[explanation_lime.top_labels[0]]
                    )

                    for j in range(len(labels_lime)):
                        if labels_lime[j] not in results_dictionary:
                            # Directly assign the value if the label is new
                            results_dictionary[labels_lime[j]] = np.array(
                                [values_lime[j]]
                            )
                        else:
                            # Concatenate the new value in a proper way
                            results_dictionary[labels_lime[j]] = np.concatenate(
                                (results_dictionary[labels_lime[j]], [values_lime[j]])
                            )
        ...
        if config["XAI_algorithm"]["BayesLime"]:
            with HiddenPrints():
                ...
                for iteration in range(config["evaluation"]["repetitions"]):
                    ...
                    labels_bayeslime, values_bayeslime, _ = zip(
                        *explanation_bayeslime.local_exp[
                            explanation_bayeslime.top_labels[0]
                        ]
                    )
                    for j in range(len(labels_bayeslime)):
                        if labels_bayeslime[j] not in results_dictionary:
                            # Directly assign the value if the label is new
                            results_dictionary[labels_bayeslime[j]] = np.array(
                                [values_bayeslime[j]]
                            )
                        else:
                            # Concatenate the new value in a proper way
                            results_dictionary[labels_bayeslime[j]] = np.concatenate(
                                (
                                    results_dictionary[labels_bayeslime[j]],
                                    [values_bayeslime[j]],
                                )
                            )
                ...
                for iteration in range(config["evaluation"]["repetitions"]):
                    ...
                    for j in range(len(labels_slime)):
                        if labels_slime[j] not in results_dictionary:
                            # Directly assign the value if the label is new
                            results_dictionary[labels_slime[j]] = np.array(
                                [values_slime[j]]
                            )
                        else:
                            # Concatenate the new value in a proper way
                            results_dictionary[labels_slime[j]] = np.concatenate(
                                (results_dictionary[labels_slime[j]], [values_slime[j]])
                            )
        ...
        if config["XAI_algorithm"]["GLIME"]:
            with HiddenPrints():
                ...
                for iteration in range(config["evaluation"]["repetitions"]):
                    ...
                    temp_normal_lime, mask_normal_lime = (
                        explanation_glime.get_image_and_mask(
                            explanation_glime.top_labels[0],
                            positive_only=config["lime_segmentation"]["positive_only"],
                            num_features=config["lime_segmentation"][
                                "num_features_explanation"
                    ...
                    for j in range(len(labels_glime)):
                        if labels_glime[j] not in results_dictionary:
                            # Directly assign the value if the label is new
                            results_dictionary[labels_glime[j]] = np.array(
                                [values_glime[j]]
                            )
                        else:
                            # Concatenate the new value in a proper way
                            results_dictionary[labels_glime[j]] = np.concatenate(
                                (results_dictionary[labels_glime[j]], [values_glime[j]])
                            )
    ...
    if config["evaluation"]["model_randomization"]:
        ...
        if config["XAI_algorithm"]["DSEG"]:
            ...
            with HiddenPrints():
                explanation = explainer_dseg.explain_instance(
                    data,
                    model_shuffled_p,
                    segmentation_algorithm[0],
                    segmentation_algorithm[1],
                ...
                temp_shuffle, mask_shuffle = explanation.get_image_and_mask(
                ...
                random_dseg_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_shuffle,
                    config=config,
                ...
                plt.imshow(
                    mark_boundaries(
                        temp_shuffle.astype(int) / 2 + 0.5, mask_shuffle.astype(int)
                    )
                )
                plt.axis("off")
                plt.savefig(
        ...
        if config["XAI_algorithm"]["LIME"]:
            ...
            with HiddenPrints():
                ...
                random_lime_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    mask_shuffle,
                    config=config,
        ...
        if config["XAI_algorithm"]["BayesLime"]:
            ...
            with HiddenPrints():
                if not config["lime_segmentation"]["all_dseg"]:
                    explanation = explainer_BayesLime.explain_instance(
                        data,
                        model_shuffled_p,
                        None,
                        None,
        ...
        if config["XAI_algorithm"]["SLIME"]:
            ...
            with HiddenPrints():
                if not config["lime_segmentation"]["all_dseg"]:
                    explanation = explainer_SLIME.explain_instance(
                        data,
                        model_shuffled_p,
                        None,
                        None,
        ...
        if config["XAI_algorithm"]["GLIME"]:
            ...
            with HiddenPrints():
                if not config["lime_segmentation"]["all_dseg"]:
                    explanation = explainer_GLIME.explain_instance(
                        data,
                        model_shuffled_p,
                        None,
                        None,
        ...
        if config["XAI_algorithm"]["EAC"]:
            ...
            with HiddenPrints():
                ...
                eac_masks = eac_explainer.get_mask(num_features)
                random_eac_prediction, data_ = predict_merge_data(
                    image_path,
                    model,
                    eac_masks,
                    config=config,
    ...
    if config["evaluation"]["explanation_randomization"]:
        ...
        if config["XAI_algorithm"]["GLIME"]:
            ...
            explainer_GLIME = explainer_GLIME = LimeImageExplainerGLIME(
                kernel_width=kernel_width, verbose=False
            )
            data, data_raw = load_and_preprocess_image(
                image_path, config, plot=False, model=model_explain_processor
            )
    ...
    if config["evaluation"]["incremental_deletion"]:
        ...
        if config["XAI_algorithm"]["DSEG"]:
            ...
            indices = get_segments(
                explanation_dseg_fix.local_exp[explanation_dseg_fix.top_labels[0]],
                len(np.unique(values_dseg)),
            )
            ...
            with HiddenPrints():
                ...
                while fraction_dseg >= 1 - config["evaluation"][
                    "incremental_deletion_fraction"
                ] and start_indices <= len(indices):
                    local_indices = indices[start_indices + 1 :]
                    start_indices += 1
                    merged_image, fraction_dseg = replace_background(
                        image_path, None, local_indices, config, data_driven=True
                    )
                    merged_image.save(to_save_placeholder)

                    result_model_dseg = predict_image(
                        to_save_placeholder,
                        model,
                        config,
                        False,
        ...
        if config["XAI_algorithm"]["LIME"]:
            ...
            with HiddenPrints():
                ...
                start_indices = 0
                old_result = ground_truth
                while fraction_lime >= 1 - config["evaluation"][
                    "incremental_deletion_fraction"
                ] and start_indices <= len(indices):
                    local_indices = indices[start_indices + 1 :]
                    start_indices += 1
                    merged_image, fraction_lime = replace_background(
                        image_path, None, local_indices, config
                    )
                    merged_image.save(to_save_placeholder)

                    result_model_lime = predict_image(
                        to_save_placeholder,
                        model,
                        config,
                        False,
        ...
        if config["XAI_algorithm"]["BayesLime"]:
            ...
            indices = get_segments(
                explanation_BayesLime_fix.local_exp[
                    explanation_BayesLime_fix.top_labels[0]
                ],
                len(np.unique(values_lime)),
            )
            area_lime = 0  # Area multiplied by the fraction of the image with confidence of model
        ...
        if config["XAI_algorithm"]["SLIME"]:
            ...
            indices = get_segments(
                explanation_SLIME_fix.local_exp[explanation_SLIME_fix.top_labels[0]],
                len(np.unique(values_lime)),
            )
            area_lime = 0  # Area multiplied by the fraction of the image with confidence of model
            with HiddenPrints():
                ...
                while fraction_lime >= 1 - config["evaluation"][
                    ...
                    merged_image, fraction_lime = replace_background(
                        ...
                        local_indices,
                        config,
                        data_driven=config["lime_segmentation"]["all_dseg"],
                    )
                    merged_image.save(to_save_placeholder)
        ...
        if config["XAI_algorithm"]["GLIME"]:
            ...
            indices = get_segments(
                explanation_GLIME_fix.local_exp[explanation_GLIME_fix.top_labels[0]],
                len(np.unique(values_lime)),
            )
            area_lime = 0  # Area multiplied by the fraction of the image with confidence of model
            with HiddenPrints():
        ...
        if config["XAI_algorithm"]["EAC"]:
            ...
            with HiddenPrints():
                ...
                while fraction_lime >= 1 - config["evaluation"][
                    "incremental_deletion_fraction"
                ] and start_indices <= len(eac_values):
                    start_indices += 1
                    data, data_raw = load_and_preprocess_image(
                        image_path, config, plot=False, model=model_explain_processor
                    )
                    ...
                    if config["model_to_explain"]["EfficientNet"]:
                        ...
                        data_transformed_lime = data_transformed_lime.transpose(1, 2, 0)
                        dim = (
                            data_transformed_lime.shape[0],
                            data_transformed_lime.shape[1],
                        )
    ...
    if config["evaluation"]["single_deletion"]:
        ...
        if config["XAI_algorithm"]["DSEG"]:
            ...
            with HiddenPrints():
                merged_image, fraction_dseg = replace_background(
                    image_path, path_background, indices, config, data_driven=True
                )
                print("DSEG", fraction_dseg, indices)
                if (
                    fraction_dseg
                    < config["evaluation"]["fraction"]
                    - config["evaluation"]["fraction_std"]
                ):
                    start_count_segments = local_features
                    while fraction_dseg < config["evaluation"]["fraction"] - config[
                        "evaluation"
                    ]["fraction_std"] and start_count_segments < len(
                        np.unique(values_lime)
                    ):
                        start_count_segments += 1
                        indices = get_segments(
                            explanation_dseg_fix.local_exp[
                                explanation_dseg_fix.top_labels[0]
                            ],
                            start_count_segments,
                        )
                        merged_image, fraction_dseg = replace_background(
                            image_path,
                            path_background,
                ...
                prediction_difference_dseg = compare_predictions(
                    ground_truth, result_model_dseg
                )

                correctness["DSEG_prediction_single_deletion"] = result_model_dseg

                if config["evaluation"]["target_discrimination"]:
                    dseg_prediction = predict_image(
                        to_save_dseg,
                        model_test,
                        config,
                        False,
        ...
        if config["XAI_algorithm"]["LIME"]:
            ...
            with HiddenPrints():
                merged_image, fraction_lime = replace_background(
                    image_path, path_background, indices, config
                )
                ...
                if (
                    ...
                    start_count_segments = local_features
                    while fraction_lime < config["evaluation"]["fraction"] - config[
                        "evaluation"
                    ]["fraction_std"] and start_count_segments < len(
                        np.unique(values_lime)
                    ):
                        start_count_segments += 1
                        indices = get_segments(
                            explanation_lime_fix.local_exp[
                                explanation_lime_fix.top_labels[0]
                            ],
                            start_count_segments,
                        )
                ...
                prediction_difference_lime = compare_predictions(
                    ground_truth, result_model_lime
                )
        ...
        if config["XAI_algorithm"]["SLIME"]:
            ...
            if config["lime_segmentation"]["adaptive_fraction"]:
                labels, values, _ = zip(
                    *explanation_SLIME_fix.local_exp[
                        explanation_SLIME_fix.top_labels[0]
                    ]
                )
        ...
        if config["XAI_algorithm"]["GLIME"]:
            ...
            if config["lime_segmentation"]["adaptive_fraction"]:
                labels, values = zip(
                    *explanation_GLIME_fix.local_exp[
                        explanation_GLIME_fix.top_labels[0]
                    ]
                )
        ...
        if config["XAI_algorithm"]["EAC"]:
            ...
            with HiddenPrints():
                ...
                def replace_background_local(
                    image_path, path_background, local_features, config, eac_explainer
                ):
                    if config["model_to_explain"]["EfficientNet"]:
                        data_transformed_lime = data.copy()
                        dim = (data.shape[0], data.shape[1])
                ...
                merged_image, fraction_lime = replace_background_local(
                    image_path, path_background, local_features, config, eac_explainer
                )
                ...
                if (
                    ...
                    while fraction_lime < config["evaluation"]["fraction"] - config[
                        "evaluation"
                    ]["fraction_std"] and start_count_segments < len(eac_values):
                        start_count_segments += 1
                        merged_image, fraction_lime = replace_background_local(
                            image_path,
                            path_background,
    ...
    if config["evaluation"]["variation_stability"]:
        ...
        if config["XAI_algorithm"]["DSEG"]:
            print("-------- DSEG --------")
            altered_data_, altered_data_raw = load_and_preprocess_image(
                image_path_altered, config, plot=False, model=model_explain_processor
            )
            if config["model_to_explain"]["EfficientNet"]:
            ...
            with HiddenPrints():
                ...
                if config["evaluation"]["preservation_check"]:
                    ...
                    plt.savefig(
                        to_save_path
                        + image_path.rsplit(".", 1)[0].split("/")[-1]
                        + "/preservation_noise_check_dseg.png",
                        bbox_inches="tight",
                        dpi=1200,
                        pad_inches=0,
                    )
                ...
                if config["evaluation"]["deletion_check"]:
                    mask_normal_dseg_inverse = 1 - np.array(mask_normal_dseg).astype(
                        bool
                    )
                    dseg_prediction, data_ = predict_merge_data(
                        image_path_altered,
        ...
        if config["XAI_algorithm"]["LIME"]:
            ...
            temp_normal_lime, mask_normal_lime = explanation_lime.get_image_and_mask(
            ...
            if config["evaluation"]["preservation_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_noise_check_lime.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
        ...
        if config["XAI_algorithm"]["BayesLime"]:
            ...
            with HiddenPrints():
                if not config["lime_segmentation"]["all_dseg"]:
                    explanation_lime = explainer_BayesLime.explain_instance(
                        altered_data,
                        model_p,
                        None,
                        None,
            ...
            if config["evaluation"]["preservation_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_noise_check_BayesLime.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
            ...
            if config["evaluation"]["deletion_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/deletion_noise_check_BayesLime.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
        ...
        if config["XAI_algorithm"]["SLIME"]:
            ...
            with HiddenPrints():
                if not config["lime_segmentation"]["all_dseg"]:
                    explanation_lime = explainer_SLIME.explain_instance(
                        altered_data,
                        model_p,
                        None,
                        None,
            ...
            if config["evaluation"]["preservation_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_noise_check_SLIME.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
            ...
            if config["evaluation"]["deletion_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/deletion_noise_check_SLIME.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
        ...
        if config["XAI_algorithm"]["GLIME"]:
            ...
            temp_normal_lime, mask_normal_lime = explanation_glime.get_image_and_mask(
                explanation_glime.top_labels[0],
                positive_only=config["lime_segmentation"]["positive_only"],
                num_features=config["lime_segmentation"]["num_features_explanation"],
                hide_rest=config["lime_segmentation"]["hide_rest"],
            ...
            if config["evaluation"]["preservation_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_noise_check_GLIME.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
            ...
            if config["evaluation"]["deletion_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/deletion_noise_check_GLIME.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
        ...
        if config["XAI_algorithm"]["EAC"]:
            ...
            if config["evaluation"]["preservation_check"]:
                ...
                plt.savefig(
                    to_save_path
                    + image_path.rsplit(".", 1)[0].split("/")[-1]
                    + "/preservation_noise_check_EAC.png",
                    bbox_inches="tight",
                    dpi=1200,
                    pad_inches=0,
                )
...
def evaluate_dataset(
    ...
    for sub_image in sub_images:
        ...
        pathlib.Path(
            "./Dataset/Results/" + date_path + "/" + sub_image.rsplit(".", 1)[0]
    ...
    pickle.dump(
        image_dictionary,
        open("./Dataset/Results/" + date_path + "/eval_results.pkl", "wb"),
    )
    # Return the image dictionary
    return image_dictionary


def evaluate_dataset_filtered(
    model_eff,
    feature_extractor,
    filtered_name,
    dataset_path,
...
def evaluate_dataset_parallel(
    ...
    parallel_data_generation = Parallel(
        n_jobs=config["computation"]["num_workers"], verbose=3, backend="loky"
    )  # loky #sequential multiprocessing
    results = parallel_data_generation(
        delayed(evaluate_sub_image)(
            sub_image,
            dataset_path,
            date_path,
            model_eff,
            ...
            if subkey:
                ...
                for subsubkey in dictionary[key][subkey].keys():
                    ...
                    if len(local_item_) > 1:
                        if len(local_item_) != 3 or len(local_item_) == 2:
                            local_item_ = local_item_[0]
                        subkey_key_dict[subsubkey + "_class"] = local_item_[1]
                        subkey_key_dict[subsubkey + "_prediction"] = round(
                            float(local_item_[2]), 4
                        )
                    else:
        ...
        if key != ".DS_Store":
            for subkey in dictionary[key].keys():
                if subkey:
                    subkey_key_dict = {}
                    for subsubkey in dictionary[key][subkey].keys():
                        local_item_ = np.array(
                            dictionary[key][subkey][subsubkey], ndmin=1
                        )
                        if len(local_item_) > 1:
                            if len(local_item_) != 3 or len(local_item_) == 2:
                                local_item_ = local_item_[0]
    ...
    for image in dictionary.keys():
        ...
        for key in dictionary[image].keys():
            if key == "groundtruth":
                ...
                for subkey in groundtruth_dict:
...
def evaluate_sub_image(
    ...
    # Create a directory for the sub-image results
    pathlib.Path(
        "./Dataset/" + folder + "/" + date_path + "/" + sub_image.rsplit(".", 1)[0]
    ).mkdir(parents=True, exist_ok=True)
...
class ColoredDF:
    ...
    def ensure_keys_and_set_default(
        self, outer_key, inner_sub_key, inner_key, default_value=1
    ):
        """
        ...
        if inner_key not in self.count_results[outer_key][inner_sub_key]:
            ...
            self.count_results[outer_key][inner_sub_key][inner_key] = (
                self.count_results[outer_key][inner_sub_key][inner_key] + 1
            )
    ...
    def color_cells_time(self, row):
        ...
        return [
            ...
            if (col in prediction_cols and value <= min_value)
        ...
        for index, row in df.iterrows():
            ...
            for col, value in row.items():
                if col in prediction_cols:
                    if "DSEG" in col:
                        if local_rule(value):
                            self.ensure_keys_and_set_default(
                                "time", "DSEG", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "time", "DSEG", "red_count"
                        ...
                        if local_rule(value):
                            self.ensure_keys_and_set_default(
                                "time", "SLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "time", "SLIME", "red_count"
                            )

                    elif "BayesLime" in col:
                        if local_rule(value):
                            self.ensure_keys_and_set_default(
                                "time", "BayesLime", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "time", "BayesLime", "red_count"
                            )

                    elif "GLIME" in col:
                        if local_rule(value):
                            self.ensure_keys_and_set_default(
                                "time", "GLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "time", "GLIME", "red_count"
                            )

                    elif "LIME" in col:
                        if local_rule(value):
                            self.ensure_keys_and_set_default(
                                "time", "LIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "time", "LIME", "red_count"
    ...
    def color_cells_preservation(self, row):
        ...
        return [
            "background-color: green"
            if (col in prediction_cols and value[0] == true_value)
            else "background-color: red"
            if col in prediction_cols
            else ""
            for col, value in row.items()
    ...
    def color_cells_output_completeness(self, row):
        ...
        colors_deletion = [
            "background-color: green"
            if (col in prediction_cols_del and value[0] != true_value_del)
            else "background-color: red"
            if col in prediction_cols_del
            else ""
            for col, value in row.items()
        ]
        combined_colors = [
            colors_preservation[i] if colors_preservation[i] else colors_deletion[i]
            for i in range(len(colors_preservation))
        ]
                ...
                if col in prediction_cols or col in prediction_cols_del:
                    if "DSEG" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "completeness", "DSEG", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "completeness", "DSEG", "red_count"
                            )

                    elif "SLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "completeness", "SLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "completeness", "SLIME", "red_count"
                            )

                    elif "BayesLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "completeness", "BayesLime", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "completeness", "BayesLime", "red_count"
                            )

                    elif "GLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "completeness", "GLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "completeness", "GLIME", "red_count"
                            )

                    elif "LIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "completeness", "LIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "completeness", "LIME", "red_count"
                            )
    ...
    def color_cells_consistency(self, row):
        ...
        colors_stability = [
            "background-color: green"
            if (col in stability_cols and value <= min_value)
            else "background-color: red"
        ...
        combined_colors = [
            colors_preservation[i]
            if colors_preservation[i]
            else colors_deletion[i]
            if colors_deletion[i]
            else colors_stability[i]
            for i in range(len(colors_preservation))
        ]
    ...
    def count_cells_consistency(self, df):
        ...
        for index, row in df.iterrows():
            ...
            for col, value in row.items():
                if (
                    col in prediction_cols
                    or col in prediction_cols_del
                    or col in stability_cols
                ):
                    if "DSEG" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "consistency", "DSEG", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "consistency", "DSEG", "red_count"
                            )
                    elif "BayesLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "consistency", "BayesLime", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "consistency", "BayesLime", "red_count"
                            )
                    elif "SLIME" in col or "SLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "consistency", "SLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "consistency", "SLIME", "red_count"
                            )

                    elif "GLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "consistency", "GLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "consistency", "GLIME", "red_count"
                            )

                    elif "LIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "consistency", "LIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "consistency", "LIME", "red_count"
                            )
    ...
    def color_cells_correctness(self, row):
        ...
        # Filter to get only columns with 'prediction' in their name
        random_cols = [
            col for col in res_df.columns if ("random" in col and "prediction" in col)
        ]
        # Calculate max value only for those columns
        true_value = row["Groundtruth"][0]
        ...
        colors_random = [
            "background-color: green"
            if (col in random_cols and value[0] != true_value)
            else "background-color: red"
        ...
        colors_deletion = [
            "background-color: green"
            if (col in single_cols and value[0] == true_value)
            else "background-color: red"
        ...
        colors_stability = [
            "background-color: green"
            if (col in compactness_cols and value <= min_value)
            else "background-color: red"
            if col in compactness_cols
            else ""
            for col, value in row.items()
        ]
        colors_incremental = [
            "background-color: green"
            if (col in incremental_cols and value <= min_incremental_value)
            else "background-color: red"
            if col in incremental_cols
            else ""
            for col, value in row.items()
        ]
        combined_colors = [
            ...
            if colors_deletion[i]
            else colors_incremental[i]
            if colors_incremental[i]
            else colors_stability[i]
            for i in range(len(colors_random))
        ]
    ...
    def count_cells_correctness(self, df):
        ...
        for index, row in df.iterrows():
            ...
            def local_rule(value, col):
                ...
                if "_Compactness" in col:
                    return value <= min_value
                if "_incremental" in col:
            ...
            for col, value in row.items():
                if col in random_cols or col in single_cols or col in compactness_cols:
                    if "DSEG" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "correctness", "DSEG", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "correctness", "DSEG", "red_count"
                            )

                    elif "SLIME" in col or "SLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "correctness", "SLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "correctness", "SLIME", "red_count"
                            )

                    elif "BayesLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "correctness", "BayesLime", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "correctness", "BayesLime", "red_count"
                            )

                    elif "GLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "correctness", "GLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "correctness", "GLIME", "red_count"
                            )

                    elif "LIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "correctness", "LIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "correctness", "LIME", "red_count"
                            )
    ...
    def color_cells_contrastivity(self, row):
        ...
        deletion_check_cols = [
            col for col in res_df.columns if ("deletion" in col and "check" in col)
        ]
        # Calculate max value only for those columns
        single_deletion_cols = [
            col for col in res_df.columns if "single_deletion" in col
        ]

        groundtruth_test_cols = [
            col for col in res_df.columns if "Groundtruth_test" in col
        ]

        colors_preservation = [
            "background-color: green"
            if (col in preservation_cols and value[0] == true_value)
            else "background-color: red"
        ...
        colors_deletion = [
            "background-color: green"
            if (col in deletion_check_cols and value[0] != true_value)
            else "background-color: red"
            if col in deletion_check_cols
            else ""
            for col, value in row.items()
        ]
        colors_single_deletion = [
            "background-color: green"
            if (col in single_deletion_cols and value[0] == true_value)
            else "background-color: red"
            if col in single_deletion_cols
            else ""
            for col, value in row.items()
        ]
        colors_groundtruth_test = [
            "background-color: green"
            if (col in groundtruth_test_cols and value[0] == true_value)
            else "background-color: red"
            if col in groundtruth_test_cols
            else ""
            for col, value in row.items()
        ]
        ...
        combined_colors = [
            ...
            if colors_deletion[i]
            else colors_single_deletion[i]
            if colors_single_deletion[i]
            else colors_groundtruth_test[i]
            for i in range(len(colors_preservation))
        ]
    ...
    def count_cells_contrastivity(self, df):
        ...
        for index, row in df.iterrows():
            ...
            def local_rule(value, col):
                if "preservation" in col:
                    return value[0] == true_value
                if "deletion" in col and "check" in col:
                    return value[0] != true_value
                if "single_deletion" in col or "Groundtruth_test" in col:
                    return value[0] == true_value

            for col, value in row.items():
                if (
                    col in preservation_cols
                    or col in deletion_check_cols
                    or col in single_deletion_cols
                    or col in groundtruth_test_cols
                ):
                    if "DSEG" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "contrastivity", "DSEG", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "contrastivity", "DSEG", "red_count"
                            )

                    elif "SLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "contrastivity", "SLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "contrastivity", "SLIME", "red_count"
                            )

                    elif "BayesLime" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "contrastivity", "BayesLime", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "contrastivity", "BayesLime", "red_count"
                            )

                    elif "GLIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "contrastivity", "GLIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "contrastivity", "GLIME", "red_count"
                            )

                    elif "LIME" in col:
                        if local_rule(value, col):
                            self.ensure_keys_and_set_default(
                                "contrastivity", "LIME", "green_count"
                            )
                        else:
                            self.ensure_keys_and_set_default(
                                "contrastivity", "LIME", "red_count"
                            )
...
def plot_evaluation(data, inclusive=True):
    ...
    pivot_df = df.pivot_table(
        index=["Category", "Method"], columns="Color", values="Count", fill_value=0
    )
    ...
    if inclusive:
        for i, (category, group_df) in enumerate(pivot_df.groupby(level=0)):
            ...
            group_df = group_df.reindex(
                [1, 0] + list(range(2, len(group_df)))
            ).reset_index(drop=True)
            group_df.set_index("Method").plot(
                kind="bar", stacked=True, ax=axes[i], color=["green", "red"]
            )
        ...
        for i, (category, group_df) in enumerate(pivot_df.groupby(level=0)):
            ...
            group_df.set_index("Method").plot(
                kind="bar", stacked=True, ax=axes[i], color=["green"]
            )
            axes[i].set_title(category.capitalize())
            axes[i].set_xlabel("Method")
            axes[i].set_ylabel("Count")
