        ...
        with torch.no_grad():
            if output == None:
                ...
                fc_res = torch.cat(
                    (
                        fc_res,
                        torch.nn.functional.softmax(fc(tmp_out), dim=1)[
                            :, target_label
                        ],
                    )
                )
...
def learn_PIE(
    ...
    bin_x = np.random.binomial(1, 0.5, size=(2500, num_feat)).astype(
        bool
    )  #### generate 2500 samples to learn PIE by default
    new_mask = np.array([masks_tmp[i].sum(0) for i in bin_x]).astype(bool)
    print(new_mask.shape)
    feat, probs = learning_feat(
        target_model, full_model, new_mask, target_img, target_label, fc, image_norm
    )
