...
def lime_segmentation(image, config):
    ...
    if config["lime_segmentation"]["slic"]:
        ...
        segments = watershed(
            image, markers=config["lime_segmentation"]["markers"], compactness=0.001
        )
...
def create_subsegment(
    segment, annotation_id, data, config, replace=True, n_segments=16
):
    """
    ...
    if config["lime_segmentation"]["auto_segment"]:
        n_segments = math.ceil(
            ...
            * fraction_of_ones_2d(binary_array)
...
def segment_image_dynamic(
    ...
    if config["lime_segmentation"].get("SAM"):
        reverse_dict = {
            v: k for k, v in links_ids.items()
        }  # Reverse the links for easier lookups
        ...
        for i in investigate_ids:
            ...
            sub_hierarchy = search_nested_dict(
                hierarchy_dict, old_key
            )  # Search for old_key in nested hierarchy

            if sub_hierarchy is not None:
                for sub_id in sub_hierarchy:
                    sub_id = int(sub_id)
                    mask = (
                        raw_Segments[sub_id].astype(np.uint8)
                        * len(np.unique(resized_panoptic_seg))
                    ) != 0
                    resized_panoptic_seg[mask] = (
                        raw_Segments[sub_id].astype(np.uint8)
                        * len(np.unique(resized_panoptic_seg))
                    )[mask]
        ...
        link_ids = {
            v: k for k, v in reverse_dict.items()
        }  # Reversing the dictionary to map new keys to original IDs

    elif static:
        for i in investigate_ids:
            resized_seg, _ = create_subsegment(
                resized_panoptic_seg, i, image, config, False, 8
            )
        ...
        for i in range(len(np.unique(resized_panoptic_seg))):
            ...
            for sub_id in np.unique(resized_seg):
                image_id_weights.append(
                    [sub_id + len(np.unique(fine_grained_segments)), i]
                )
...
def segment_seed(image, image_path, config, feature_extractor, model, dim):
    """
    ...
    if config["lime_segmentation"]["DETR"]:
        ...
        result = feature_extractor.post_process_panoptic_segmentation(
            outputs, target_sizes=[dim]
        )
...
def segment_seed_dynamic(image, image_path, config, feature_extractor, model, dim):
    """
    ...
    if config["lime_segmentation"]["DETR"]:
        ...
        resized_panoptic_seg_mask = create_mask_sam(
            resized_panoptic_seg_, hierarchical_dict
        )
...
def remove_small_clusters(mask_data, min_area, plot=False):
    ...
    if plot:
        print(sorted_dict)
    keys_to_delete = [key for key in sorted_dict if key < min_area]
    if plot:
    ...
    for i in range(len(ids_list)):
        for j in range(len(ids_list) - i):
            j += i
            overlaps[i][j] = np.round(
                calculate_iou(
                    mask_data[ids_list[i]]["segmentation"],
                    mask_data[ids_list[j]]["segmentation"],
                ),
                2,
            )
    ...
    if plot:
        ax = sns.heatmap(
            overlaps, linewidth=0.5, xticklabels=ids_list, yticklabels=ids_list
        )
        plt.show()
    ...
    for node in G.nodes():
        ...
        if len(incoming_edges) > 1:
            ...
            if all(weight == weights[0] for weight in weights):  # All weights equal
                longest_distance_edge = max(
                    incoming_edges,
                    key=lambda x: shortest_paths.get(x[0], float("-inf")),
                )
...
def create_mask_sam(sam_mask_raw, hierarchical_dict):
    ...
    raw_mask = np.zeros(
        (sam_mask_raw[zero_key].shape[0], sam_mask_raw[zero_key].shape[1])
    )
...
def fill_with_nearest(image, distance_threshold=10, size_threshold=500):
    ...
    distances, nearest_label = ndimage.distance_transform_edt(
...
def merge_small_segments(segments, size_threshold):
    ...
    small_segments = [
        segment for segment, size in segment_sizes.items() if size < size_threshold
    ]
    # Create a mask for small segments
    background = np.isin(segments, small_segments)

    nearest_label = ndimage.distance_transform_edt(
