...
from sklearn.linear_model import BayesianRidge, Lars, Ridge, lars_path
...
class LimeBase(object):
    ...
    def explain_instance_with_data(
        ...
        if model_regressor == "Lars":
            model_reg = Lars(fit_intercept=True, random_state=self.random_state)
            ...
            model_reg = BayesianRidge_inf_prior(
                fit_intercept=True,
                n_iter=0,
                tol=0.0001,
            ...
            model_reg = BayesianRidge_inf_prior_fit_alpha(
                fit_intercept=True,
                n_iter=1000,
                tol=0.0001,
                lambda_init=lambda_init,
        ...
        if model_regressor != "Lars":
            ...
            easy_model.fit(
                neighborhood_data[:, used_features], labels_column
            )  # , sample_weight=weights)
            prediction_score = easy_model.score(
                neighborhood_data[:, used_features], labels_column
            )  # , sample_weight=weights)
        ...
        if (
            ...
            or model_regressor == "Bayes_ridge"
            or model_regressor == "BayesianRidge_inf_prior_fit_alpha"
        ):
            print("the alpha is", easy_model.alpha_)
            print("the lambda is", easy_model.lambda_)
            print(
                "the regulation term lambda/alpha is",
                easy_model.lambda_ / easy_model.alpha_,
            )
            local_pred, local_std = easy_model.predict(
                neighborhood_data[0, used_features].reshape(1, -1), return_std=True
            )
        ...
        if model_regressor == None or model_regressor == "Lars":
            return (
                ...
                sorted(
                    zip(
                        used_features, easy_model.coef_, np.zeros(len(easy_model.coef_))
                    ),
                    key=lambda x: np.abs(x[1]),
                    reverse=True,
                ),
        ...
        return (
            easy_model.intercept_,
            sorted(
                zip(used_features, easy_model.coef_, variance),
                key=lambda x: np.abs(x[1]),
                reverse=True,
            ),
            prediction_score,
...
class DSEG_Lime(object):
        ...
        for iter in range(iterations):
            ...
            if iter == 0:
                segments_seed, raw_Segments, hierarchy_dict, links_ = (
                    self.segment_seed_dynamic(dim=dim_local)
                )
                ...
                segments_seed = self.segment_image_dynamic(
                    segments_seed, top_labels_list, raw_Segments, hierarchy_dict, links_
                )
            ...
            top_labels_list = ret_exp.local_exp[ret_exp.top_labels[0]][
                0 : self.num_features_explanation
            ]
        return ret_exp
    ...
    def segment_seed_dynamic(self, dim, sam_model=True):
        """
        ...
        if not sam_model:
            ...
            result = self.feature_extractor.post_process_panoptic_segmentation(
            ...
            hierarchical_dict = {
                root: self.build_hierarchy(graph, root) for root in roots
            }
            ...
            resized_panoptic_seg_mask = self.create_mask_sam(
                resized_panoptic_seg_, hierarchical_dict, iteration=0, seeds=[]
            )
    ...
    def segment_image_dynamic(
        self, segmented_image, annotation_ids, raw_Segments, hierarchy_dict, links_ids
    ):
        """
        ...
        for i in investigate_number:
            ...
            if old_key in hierarchy_dict.keys():
                for j in hierarchy_dict[old_key]:
                    mask = (
                        raw_Segments[int(j)].astype(np.uint8)
    ...
    def create_mask_sam(self, sam_mask_raw, hierarchical_dict, iteration=0, seeds=None):
        ...
        for key in hierarchical_dict.keys():
            raw_mask = add_mask(
                raw_mask, alter_values(sam_mask_raw[int(key)], int(key))
            )
