...
def get_aligned_paths(video_dir, audio_dir, caption_dir):
    """
    Returns aligned video, audio, caption paths
    based on filename intersection.
    ...
    Returns:
    - tuple: (video_paths, audio_paths, caption_paths) where each is a list of file paths
    """
    ...
    video_paths = [os.path.join(video_dir, f"{id}.mp4") for id in common_ids]
    audio_paths = [os.path.join(audio_dir, f"{id}.wav") for id in common_ids]
    caption_paths = [os.path.join(caption_dir, f"{id}.srt") for id in common_ids]
