...
PLANNER_PROMPT_PATH = Path(__file__).parent / "prompts" / "planner_finqa.txt"
...
def build_planner_prompt(sample: str) -> str:
    """
...
class PlannerAgent:
    ...
    def run(self, sample: str, lf_trace: Any = None) -> Tuple[str, dict, bool, str]:
        """
        ...
        agent = Agent(
            role="Financial report calculator planner",
            goal=(
                "Produce a precise JSON calculation plan for answering a question about a financial report. "
                "Output JSON only — no extra text."
            ),
            backstory=(
                "You are an expert financial analyst. You plan structured calculation procedures "
                "so a calculation agent can follow your steps precisely."
            ),
            llm=self._llm,
            verbose=False,
        ...
        task = Task(
            ...
            expected_output=(
                ...
                "expected_answer_type, answerability_check, hints"
            ),
            agent=agent,
        )
